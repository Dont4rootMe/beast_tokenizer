...
def objective(trial, /, dataloader, num_dof, seq_len, max_samples_train, max_samples_validate):
    ...
    vocab_size = trial.suggest_int("vocab_size", 150, 15_000)
    degree_p = trial.suggest_int("degree_p", 0, 5)
    ...
    for batch in dataloader:
        ...
        b = batch['actions']
        error = tokenizer.compute_reconstruction_error(b)
        errors.append(error)
...
def main():
    ...
    dtl = DataLoader(robotics_dataset, batch_size=36, shuffle=True)
    ...
    study.optimize(obj_func, n_trials=200, n_jobs=-1)
