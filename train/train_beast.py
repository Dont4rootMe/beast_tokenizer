...
from train.data import (
    BEAST_TRAIN_MAX_SAMPLES,
    BPE_TRAIN_MAX_SAMPLES,
    prepare_dataloader,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--num-basis", type=int, default=32, help="Number of spline basis functions."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="cpu",
        help="Device used for fitting (cpu or cuda).",
    )
    parser.add_argument(
        "--gripper-indices",
        type=int,
        nargs="*",
        default=(),
        help="Optional gripper DoF indices.",
    )
    parser.add_argument(
        "--beast-max-samples",
        type=int,
        default=BEAST_TRAIN_MAX_SAMPLES,
        help="Number of sequences for BEAST parameter fitting.",
    )
    parser.add_argument(
        "--max-samples",
        type=int,
        default=BPE_TRAIN_MAX_SAMPLES,
        help="Number of dataloader batches used for BPE fitting.",
    )
    parser.add_argument(
        "--bpe-vocab-size",
        type=int,
    ...
    parser.add_argument(
        "--beast-checkpoint-dir",
        type=str,
        default="beast_tokenizer_checkpoint",
        help="Directory to store the base tokenizer.",
    )
    parser.add_argument(
        "--bpe-checkpoint-dir",
        type=str,
        default="beast_bpe_tokenizer_checkpoint",
        help="Directory to store the BPE tokenizer.",
    )
    train_bpe_group = parser.add_mutually_exclusive_group()
    train_bpe_group.add_argument(
        "--train-bpe",
        dest="train_bpe",
        action="store_true",
        help="Train the BPE tokenizer (enabled by default).",
    )
    train_bpe_group.add_argument(
        "--no-train-bpe",
        dest="train_bpe",
        action="store_false",
        help="Skip the BPE tokenizer fitting stage.",
    )
    parser.set_defaults(train_bpe=True)
    args = parser.parse_args()

    robotics_dataset, dataloader = prepare_dataloader(args.batch_size)
    ...
    tokenizer.fit_parameters(dataloader, max_samples=args.beast_max_samples)
    ...
    limited_batches: Iterable[Any] = _limit_batches(dataloader, args.max_samples)
